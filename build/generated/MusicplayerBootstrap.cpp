...
#define TAG "com.activate.musicplayer"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Musicplayer_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Musicplayer.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::MusicplayerBindings::lookupGeneratedInit(
...
static void Musicplayer_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Musicplayer_getBinding)->GetFunction());
}

static void Musicplayer_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::MusicplayerBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MusicplayerBinding = {
	"com.activate.musicplayer",
	Musicplayer_init,
	Musicplayer_dispose
};
...
Java_com_activate_musicplayer_MusicplayerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.activate.musicplayer", &MusicplayerBinding);
	titanium::KrollBindings::addExternalLookup(&(::MusicplayerBindings::lookupGeneratedInit));
}
